...
load('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\Results Time ALL.mat')
% load('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\Results Time MAIN.mat')
% load('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\Results Time Binary.mat')

load('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\Results Time MAIN.mat','seqsolDelayed', 'Att_BR_Delayed_acyclic',...
    'Att_MS_Delayed_acyclic','Attention_pref')

N=145;

%% WRI Identification

testBR = zeros(size(Time_Ranking,3),1);
testCRP = zeros(size(Time_Ranking,3),1);
testCC = zeros(size(Time_Ranking,3),1);
testMS = zeros(size(Time_Ranking,3),1);
testEIG = zeros(size(Time_Ranking,3),1);
testSEQ = zeros(size(Time_Ranking,3),1);
testTC = zeros(size(Time_Ranking,3),1);
testTC_BR = zeros(size(Time_Ranking,3),1);
testATT = zeros(size(Time_Ranking,3),1);
testATT_BR = zeros(size(Time_Ranking,3),1);
testATT_MS = zeros(size(Time_Ranking,3),1);

for i = 1:size(Time_Ranking,3)
    testBR(i,1) = isequal(Time_Ranking(:,:,i),BernRangDelayed(:,:,i));
end
for i = 1:size(Time_Ranking,3)
    testCRP(i,1) = isequal(Time_Ranking(:,:,i),countrevsolDelayed(:,:,i));
end
for i = 1:size(Time_Ranking,3)
    testCC(i,1) = isequal(Time_Ranking(:,:,i),countsolDelayed(:,:,i));
end
for i = 1:size(Time_Ranking,3)
    testMS(i,1) = isequal(Time_Ranking(:,:,i),minswapssolDelayed(:,:,i));
end
for i = 1:size(Time_Ranking,3)
    testEIG(i,1) = isequal(Time_Ranking(:,:,i),eigensolDelayed(:,:,i));
end
for i = 1:size(Time_Ranking,3)
    testSEQ(i,1) = isequal(Time_Ranking(:,:,i),seqsolDelayed(:,:,i));
end
for i = 1:size(Time_Ranking,3)
    testTC(i,1) = isequal(Time_Ranking(:,:,i),trcoreDelayed(:,:,i));
end
for i = 1:size(Time_Ranking,3)
    testTC_BR(i,1) = isequal(Time_Ranking(:,:,i),trcoreDelayed_BR(:,:,i));
end
for i = 1:size(Time_Ranking,3)
    testATT(i,1) = isequal(Time_Ranking(:,:,i),Attention_pref(:,:,i));
end
for i = 1:size(Time_Ranking,3)
    testATT_BR(i,1) = isequal(Time_Ranking(:,:,i),Att_BR_Delayed_acyclic(:,:,i));
end
for i = 1:size(Time_Ranking,3)
    testATT_MS(i,1) = isequal(Time_Ranking(:,:,i),Att_MS_Delayed_acyclic(:,:,i));
end
...
% save('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\WRI Identification Time ALL','WRI_BR','WRI_CC','WRI_CRP','WRI_EIG','WRI_TC','WRI_SEQ','WRI_MS','WRI_TC_BR','WRI_ATT','WRI_ATT_BR','WRI_ATT_MS');
save('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\WRI Identification Time MAIN','WRI_BR','WRI_CC','WRI_CRP','WRI_EIG','WRI_TC','WRI_SEQ','WRI_MS','WRI_TC_BR','WRI_ATT','WRI_ATT_BR','WRI_ATT_MS');
% save('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\WRI Identification Time Binary','WRI_BR','WRI_CC','WRI_CRP','WRI_EIG','WRI_TC','WRI_SEQ','WRI_MS','WRI_TC_BR','WRI_ATT','WRI_ATT_BR','WRI_ATT_MS');
...
for z=1:N
    for i=1:4
       if sum(countrevsolDelayed(i,:,z))==3
           winner_CRP(z,i)=1;
       else
           winner_CRP(z,i)=0;
       end
...
for z=1:N
    for i=1:4
       if sum(minswapssolDelayed(i,:,z))==3
           winner_MS(z,i)=1;
       else
           winner_MS(z,i)=0;
       end
...
for z=1:N
    for i=1:4
       if sum(Att_BR_Delayed_acyclic(i,:,z))==3
           winner_ATT_BR(z,i)=1;
       else
           winner_ATT_BR(z,i)=0;
       end
...
for z=1:N
    for i=1:4
       if sum(Att_MS_Delayed_acyclic(i,:,z))==3
           winner_ATT_MS(z,i)=1;
       else
           winner_ATT_MS(z,i)=0;
       end
...
% save('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\UI Identification Time ALL','UI_BR','UI_CC','UI_CRP','UI_EIG','UI_TC','UI_SEQ','UI_MS','UI_ATT_MS','UI_ATT_BR','UI_ATT','UI_TC_BR');
save('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\UI Identification Time MAIN','UI_BR','UI_CC','UI_CRP','UI_EIG','UI_TC','UI_SEQ','UI_MS','UI_ATT_MS','UI_ATT_BR','UI_ATT','UI_TC_BR');
% save('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\UI Identification Time Binary','UI_BR','UI_CC','UI_CRP','UI_EIG','UI_TC','UI_SEQ','UI_MS','UI_ATT_MS','UI_ATT_BR','UI_ATT','UI_TC_BR');


%% Expected Identification

%%%%%%%%%% Bernheim and Rangel, and the attention models, and the transitive core, has to be completed in order to calculate 
...
for z=1:size(BernRangDelayed,3)
for i=1:4
for j=1:4
     if BernRangDelayed(i,j,z)==BernRangDelayed(j,i,z) && i ~=j
        BernRangDelayed(i,j,z)=1;
        BernRangDelayed(j,i,z)=1;
     end
     if Attention_pref(i,j,z)==Attention_pref(j,i,z) && i ~=j
        Attention_pref(i,j,z)=1;
        Attention_pref(j,i,z)=1;
     end   
     if Att_MS_Delayed_acyclic(i,j,z)==Att_MS_Delayed_acyclic(j,i,z) && i ~=j
        Att_MS_Delayed_acyclic(i,j,z)=1;
        Att_MS_Delayed_acyclic(j,i,z)=1;
     end     
     if Att_BR_Delayed_acyclic(i,j,z)==Att_BR_Delayed_acyclic(j,i,z) && i ~=j
        Att_BR_Delayed_acyclic(i,j,z)=1;
        Att_BR_Delayed_acyclic(j,i,z)=1;
     end  
     if trcoreDelayed(i,j,z)==0 && trcoreDelayed(j,i,z)==0 && i~=j
        trcoreDelayed(i,j,z)=1;
        trcoreDelayed(j,i,z)=1;
     end     
     if trcoreDelayed_BR(i,j,z)==0 && trcoreDelayed_BR(j,i,z)==0 && i~=j
        trcoreDelayed_BR(i,j,z)=1;
        trcoreDelayed_BR(j,i,z)=1;
     end
end
end
end
...
% save('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\EI Identification Time ALL','EI_BR','EI_CC','EI_CRP','EI_EIG','EI_TC','EI_SEQ','EI_MS','EI_TC_BR','EI_ATT_BR','EI_ATT_MS','EI_ATT');
save('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\EI Identification Time MAIN','EI_BR','EI_CC','EI_CRP','EI_EIG','EI_TC','EI_SEQ','EI_MS','EI_TC_BR','EI_ATT_BR','EI_ATT_MS','EI_ATT');
% save('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\EI Identification Time Binary','EI_BR','EI_CC','EI_CRP','EI_EIG','EI_TC','EI_SEQ','EI_MS','EI_TC_BR','EI_ATT_BR','EI_ATT_MS','EI_ATT');


%% Symmetric difference and Reverse Asymmetry

Diff_CRP = countrevsolDelayed - Time_Ranking;
Diff_CC = countsolDelayed - Time_Ranking;
Diff_EIG = eigensolDelayed - Time_Ranking;
Diff_SEQ = seqsolDelayed - Time_Ranking;
Diff_TC = trcoreDelayed - Time_Ranking;
Diff_BR = BernRangDelayed - Time_Ranking;
Diff_MS = minswapssolDelayed - Time_Ranking;
Diff_TC_BR = trcoreDelayed_BR - Time_Ranking;
Diff_ATT = Attention_pref - Time_Ranking;
Diff_ATT_MS = Att_MS_Delayed_acyclic - Time_Ranking;
Diff_ATT_BR = Att_BR_Delayed_acyclic - Time_Ranking;
...
% save('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\SD Identification Time ALL','SD_BR','SD_CC','SD_CRP','SD_EIG','SD_TC','SD_SEQ','SD_MS','SD_TC_BR','SD_ATT','SD_ATT_MS','SD_ATT_BR');
save('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\SD Identification Time MAIN','SD_BR','SD_CC','SD_CRP','SD_EIG','SD_TC','SD_SEQ','SD_MS','SD_TC_BR','SD_ATT','SD_ATT_MS','SD_ATT_BR');
% save('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\SD Identification Time Binary','SD_BR','SD_CC','SD_CRP','SD_EIG','SD_TC','SD_SEQ','SD_MS','SD_TC_BR','SD_ATT','SD_ATT_MS','SD_ATT_BR');
...
% save('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\RA Identification Time ALL','RA_BR','RA_CC','RA_CRP','RA_EIG','RA_TC','RA_SEQ','RA_MS','RA_TC_BR','RA_ATT','RA_ATT_BR','RA_ATT_MS');
save('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\RA Identification Time MAIN','RA_BR','RA_CC','RA_CRP','RA_EIG','RA_TC','RA_SEQ','RA_MS','RA_TC_BR','RA_ATT','RA_ATT_BR','RA_ATT_MS');
% save('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\RA Identification Time Binary','RA_BR','RA_CC','RA_CRP','RA_EIG','RA_TC','RA_SEQ','RA_MS','RA_TC_BR','RA_ATT','RA_ATT_BR','RA_ATT_MS');
...
for i=1:145
    if m_BR(i)==0
        set_BR(i)=0;
        for j=1:24
            temp = BernRangDelayed(:,:,i) - linear(:,:,j);
...
for i=1:145
    if m_MS(i)==0
        set_MS(i)=0;
        for j=1:24
            temp = minswapssolDelayed(:,:,i) - linear(:,:,j);
            if isempty(temp(temp<0))
            set_MS(i) = set_MS(i) + 1;
            else
            end
...
for i=1:145
    if m_EIG(i)==0
        set_EIG(i)=0;
        for j=1:24
            temp = eigensolDelayed(:,:,i) - linear(:,:,j);
            if isempty(temp(temp<0))
...
for i=1:145
    if m_CC(i)==0
        set_CC(i)=0;
        for j=1:24
            temp = countsolDelayed(:,:,i) - linear(:,:,j);
            if isempty(temp(temp<0))
            set_CC(i) = set_CC(i) + 1;
            else
            end
...
for i=1:145
    if m_CRP(i)==0
        set_CRP(i)=0;
        for j=1:24
            temp = countrevsolDelayed(:,:,i) - linear(:,:,j);
            if isempty(temp(temp<0))
...
for i=1:145
    if m_TC_BR(i)==0
        set_TC_BR(i)=0;
        for j=1:24
            temp = trcoreDelayed_BR(:,:,i) - linear(:,:,j);
            if isempty(temp(temp<0))
...
for i=1:145
    if m_ATT_BR(i)==0
        set_ATT_BR(i)=0;
        for j=1:24
            temp = Att_BR_Delayed_acyclic(:,:,i) - linear(:,:,j);
            if isempty(temp(temp<0))
...
for i=1:145
    if m_ATT_MS(i)==0
        set_ATT_MS(i)=0;
        for j=1:24
            temp = Att_MS_Delayed_acyclic(:,:,i) - linear(:,:,j);
            if isempty(temp(temp<0))
...
save('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\SET_WRI Identification Time ALL','exp_BR','exp_CC','exp_CRP','exp_EIG','exp_TC','exp_SEQ','exp_MS','exp_TC_BR','exp_ATT','exp_ATT_BR','exp_ATT_MS');
% save('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\SET_WRI Identification Time MAIN','exp_BR','exp_CC','exp_CRP','exp_EIG','exp_TC','exp_SEQ','exp_MS','exp_TC_BR','exp_ATT','exp_ATT_BR','exp_ATT_MS');
% save('C:\Users\caliari\Dropbox\JOB MARKET PAPER\Codes\Identification\Identification Data\SET_WRI Identification Time Binary','exp_BR','exp_CC','exp_CRP','exp_EIG','exp_TC','exp_SEQ','exp_MS','exp_TC_BR','exp_ATT','exp_ATT_BR','exp_ATT_MS');
